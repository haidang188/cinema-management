...
function formatDiscount(item) {
  if (item.discountValue === null || item.discountValue === undefined) return '-'

  if (item.discountType === 'PERCENTAGE') {
    return `${new Intl.NumberFormat('vi-VN').format(item.discountValue)}%`
  }
...
export function PromotionList() {
  ...
  const visiblePages = useMemo(
    () => getVisiblePages(data?.page ?? 0, data?.totalPages ?? 0),
    [data?.page, data?.totalPages]
  )

  const firstItem = data?.totalElements
    ? data.page * data.size + 1
    : 0

  const lastItem = data?.totalElements
  ...
  return (
    <section className="promotion-page">
      {toast && (
        <div className="promotion-toast success">✓ {toast}</div>
      )}

      <div className="promotion-page-header premium-header">
        <div className="promotion-header-copy">
          <span className="promotion-eyebrow">PREMIERE CINEMAS / PROMOTIONS</span>
          <h1>Quản lý khuyến mãi</h1>
          {/* <p>
            Theo dõi, kiểm soát và quản lý các chương trình ưu đãi đang vận hành
            trong hệ thống Premiere Cinemas.
          </p> */}
        </div>
      ...
      <div className="promotion-section-heading">
        <div>
          <span className="promotion-section-kicker">OVERVIEW</span>
          <h2>Tổng quan chương trình</h2>
        </div>
        <p>Dữ liệu tổng hợp trên toàn bộ hệ thống.</p>
      </div>
      ...
      <div className="promotion-stat-grid promotion-stat-grid-four">
        <article className="promotion-stat-card">
          ...
          <small>Chương trình toàn hệ thống</small>
        </article>
        ...
        <article className="promotion-stat-card accent">
          ...
          <small>Khuyến mãi còn hiệu lực</small>
        </article>
        ...
        <article className="promotion-stat-card">
          ...
          <small>Chương trình chưa bắt đầu</small>
        </article>
        ...
        <article className="promotion-stat-card">
          ...
          <strong>{statistics?.expiringSoon ?? '--'}</strong>
          <small>Kết thúc trong 7 ngày</small>
        </article>
      </div>

      <div className="promotion-section-heading promotion-list-heading">
        <div>
          <span className="promotion-section-kicker">PROMOTION DIRECTORY</span>
          <h2>Danh sách khuyến mãi</h2>
        </div>
        <p>Tìm kiếm và lọc nhanh chương trình theo nhu cầu quản trị.</p>
      </div>
      ...
      <div className="promotion-directory-panel">
        ...
        <div className="promotion-table-panel premium-table-panel">
          {loading && <div className="promotion-state">Đang tải dữ liệu...</div>}
          {error && <div className="promotion-state error">{error}</div>}
                ...
                <table className="promotion-table promotion-table-modern">
                  ...
                  <tbody>
                    {data.content.map((item) => {
                      const usagePercent = item.usageLimit
                        ? Math.min(100, ((item.usedCount ?? 0) / item.usageLimit) * 100)
                        : 0
                        ...
                        <tr key={item.id}>
                          <td>
                            <div className="promotion-main-cell">
                              <div className="promotion-image-box promotion-image-box-wide">
                                {item.imageUrl ? (
                                  <img src={toAssetUrl(item.imageUrl)} alt={item.title} />
                                ) : (
                                  <span>NO IMAGE</span>
                                )}
                              </div>
                              ...
                              <div className="promotion-title-cell promotion-title-cell-rich">
                                <span className="promotion-code-tag">
                                  {item.code || `ID #${String(item.id).padStart(3, '0')}`}
                                </span>
                                <strong>{item.title}</strong>
                                <small>{item.description || 'Chưa có mô tả chương trình.'}</small>
                              </div>
                            </div>
                          </td>
                          ...
                          <td>
                            <div className="promotion-discount-cell promotion-discount-cell-modern">
                              <strong className="discount-value">{formatDiscount(item)}</strong>
                              <small>
                          ...
                          <td>
                            <span className={`promotion-status ${String(item.status || '').toLowerCase()}`}>
                              <i />
              ...
              <div className="promotion-pagination promotion-pagination-modern">
                <div className="pagination-summary pagination-summary-rich">
                  <span>
                    ...
                    <strong>{data.totalElements}</strong> khuyến mãi
                  </span>
                ...
                <div className="pagination-buttons">
                  <button disabled={data.first} onClick={() => setPage((p) => Math.max(0, p - 1))}>‹</button>
                  ...
                  {visiblePages.at(-1) < data.totalPages - 1 && (
                    <>
                      {visiblePages.at(-1) < data.totalPages - 2 && (
                        <span className="pagination-ellipsis">…</span>
                      )}
                      <button onClick={() => setPage(data.totalPages - 1)}>{data.totalPages}</button>
                    </>
                  )}

                  <button disabled={data.last} onClick={() => setPage((p) => p + 1)}>›</button>
                </div>
              </div>
            </>
          )}
